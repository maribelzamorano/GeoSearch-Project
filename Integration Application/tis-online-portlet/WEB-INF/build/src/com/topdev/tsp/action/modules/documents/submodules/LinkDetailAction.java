...
public class LinkDetailAction extends DetailAction
{
	...
	{
		super.init(aktion);
		entries = getMainObject().getLinkList().getEntryList();

		entry	= (Link) getElementById(entries);
		if(entry==null) entry = new Link(1);
		else if(!entry.getId_entity().equals("0"))
			getMainObject().loadDataLinkSublists(entry);

	}
	public String insert() throws Exception
	{
		init("insert");
		entry = new Link(getMaxId(entries), 1);
		return SUCCESS;
	}
	public String delete() throws Exception
	{
		init("delete");
		entry.setDeleted("1");
		getMainObject().deleteLink(entry);
		if(getMainObject().getErrors().size()<1) getMainObject().loadLinks();
	...
	public String save() throws Exception
	{
		...
		getMainObject().saveLink(entry);
		if(getMainObject().getErrors().size()>0) setRtErrors(getMainObject().getErrors());
		else getMainObject().loadLinks();
	...
	public Link getEntry() {
		return entry;
	}
	public void setEntry(Link entry) {
		this.entry = entry;
	}
}