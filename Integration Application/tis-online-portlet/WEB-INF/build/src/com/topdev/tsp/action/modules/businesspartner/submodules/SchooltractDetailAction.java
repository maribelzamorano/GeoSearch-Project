...
public class SchooltractDetailAction extends DetailAction
{
	private static final long	serialVersionUID	= 1L;
	OverviewSchooltract entry;
	List<ComplexEntryList> entries;

	public void init(String aktion) throws Exception
	{
		super.init(aktion);
		entries = getMainObject().getSchooltractList().getEntryList();
		entry = (OverviewSchooltract) getElementById(entries);
		if(entry==null) entry = new OverviewSchooltract();
	}
	public String insert() throws Exception
	{
		init("insert");
		entry = new OverviewSchooltract(getMaxId(entries));
		return SUCCESS;
	...
	public String save() throws Exception
	{
		...
		{
			entry = new OverviewSchooltract(getActionParameters());
			entry.setInserted("1");
			entries.add(entry);
		}
...
//			entry.setAdressart_decode(DropdownContentAction.getDecodeByCode("ANAR", entry.getAdressart()));
	...
	public OverviewSchooltract getEntry() {
		return entry;
	}
	public void setEntry(OverviewSchooltract entry) {
		this.entry = entry;
	}
