...
public class ProjectDetailAction extends DetailAction
{
	...
	public void init(String aktion) throws Exception
	{
		...
		entry = (SchoolLifeProject) getElementById(entries);
		if(entry==null) entry = new SchoolLifeProject();
		else if(!entry.getId_entity().equals("0"))
		{
			entry.getSub_list1().getEntryList().clear();
			entry.getSub_list2().getEntryList().clear();
			getMainObject().loadDataSublist1(entry, "FJEI", "EIPR");
			getMainObject().loadDataSublist2(entry, "KJEI", "EIPR");
			};
	}
	...
	{
		init("insert");
		entry = new SchoolLifeProject(getMaxId(entries));
		return SUCCESS;
	...
	public String delete() throws Exception
	{
		init("delete");
		entry.setDeleted("1");
		getMainObject().saveDataProject(entry);
	...
	public String save() throws Exception
	{
		...
		{
			entry = new SchoolLifeProject(getActionParameters());
			entry.setInserted("1");
			entries.add(entry);
	...
	public SchoolLifeProject getEntry()
	{
		return entry;
	}

	public void setEntry(SchoolLifeProject entry)
	{
		this.entry = entry;
	}
