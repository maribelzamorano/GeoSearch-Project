...
public class ContestDetailAction extends DetailAction
{
	...
	public void init(String aktion) throws Exception
	{
		...
		if(getMainObject().getContestList()==null) getMainObject().loadDataContest();
		entries = getMainObject().getContestList().getEntryList();
		entry = (Contest) getElementById(entries);
		if(entry==null) entry = new Contest();
	}
	...
	{
		init("insert");
		entry = new Contest(getMaxId(entries));
		return SUCCESS;
	...
	public String save() throws Exception
	{
		...
		{
			entry = new Contest(getActionParameters());
			entry.setInserted("1");
			entries.add(entry);
		}
	...
	public Contest getEntry()
	{
		return entry;
	}

	public void setEntry(Contest entry)
	{
		this.entry = entry;
	}
