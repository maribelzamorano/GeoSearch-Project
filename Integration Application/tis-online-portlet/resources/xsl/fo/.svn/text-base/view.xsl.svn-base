	...
	<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
		<fo:layout-master-set>
			<fo:simple-page-master
				...
				margin-left="1.8cm"
				margin-right="1.8cm">
				<fo:region-body margin-top="2.3cm" margin-bottom="0cm" />
				<fo:region-before extent="3.6cm"/>
				<fo:region-after/>
			</fo:simple-page-master>
		</fo:layout-master-set>
		...
		<fo:page-sequence master-reference="simpleA4">
			<fo:static-content flow-name="xsl-region-before">
				<fo:block border-bottom="1pt #9EBED1 solid">
			...
			<fo:flow flow-name="xsl-region-body">
				<fo:block font-size="10pt" padding-left="0.5cm" padding-right="0.5cm" padding-bottom="0.2cm" space-after="5mm">
					<xsl:apply-templates />
				</fo:block>
			</fo:flow>
		</fo:page-sequence>
...
<xsl:template match="table">
	<fo:table table-layout="fixed" width="100%" border-collapse="separate" keep-together.within-page="always" margin-bottom="0.3cm">
...
<xsl:template match="line">
	<fo:block font-size="10pt" text-align="justify" language="en" hyphenate="true">
		<xsl:if test="@space-after!=''"><xsl:attribute name="space-after"><xsl:value-of select="@space-after"/></xsl:attribute></xsl:if>
		<xsl:if test="@space-before!=''"><xsl:attribute name="space-before"><xsl:value-of select="@space-before"/></xsl:attribute></xsl:if>
		<xsl:apply-templates/>
	</fo:block>
...
<xsl:template match="title">
	<fo:block font-size="10pt" font-style="italic" color="#666666" keep-with-next.within-page="always" margin-bottom="3mm">
		<xsl:if test="@space-after!=''"><xsl:attribute name="space-after"><xsl:value-of select="@space-after"/></xsl:attribute></xsl:if>
		<xsl:if test="@space-before!=''"><xsl:attribute name="space-before"><xsl:value-of select="@space-before"/></xsl:attribute></xsl:if>
		<xsl:if test="@space!=''">
