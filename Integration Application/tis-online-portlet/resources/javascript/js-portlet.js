...
//Dialogfunktionen
	...
	{
		//alert("https://topdevcl001/tis-online-portlet"+urlPart);
	...
	showRequestDialog : function(ns, text)
	{
		try { window.scrollTo(0, 0); } catch(e){}

		if(text) document.getElementById('tsp_portlet_div_request_text_'+ns).innerHTML = text;
		var html = document.getElementById('tsp_portlet_div_request_'+ns).innerHTML; //'<input class="button" id="ok_btn" value="OK" onclick="Liferay.Popup.close(this);" type="button">';
...
/*
		...
		{
			document.getElementById('tsp_portlet_div_request_'+ns).style.display = "inline";
			if(text) document.getElementById('tsp_portlet_div_request_text_'+ns).innerHTML = text;
			//alert(document.getElementById('tsp_portlet_div_request_'+ns).style.display);
		}
		else eval("tsp_dialog_request_"+ns+"();");
		*/
...
//Listenfunktionen
	...
	addEntry : function(ajaxId, listName)
	{
		...
		tr.className="topdev_row_inserted";
...
//		document.getElementById(tabId).innerHTML="wird geladen..."
...
//			alert("complete1")
		...
		AjaxUtil.update(url, tabId, options);
	},
	
	updateDiv : function(divId, url, pF)
	{
		
//		document.getElementById(tabId).innerHTML="wird geladen..."
		var options = new Object();
//		options.method="get";
		if(pF)	options.onComplete = pF;
		else 	options.onComplete = function(req, args)
		{
			updateDiv_ext();
//			alert("complete2")
		}
		AjaxUtil.update(url+"&ajaxId="+divId, divId, options);
	...
	ogCheck : function(field, ns, ajaxId, noObligation)
	{
		...
		else
		{
			...
			{
				document.getElementById(field.id+"_td").className="obligation";
				og = true;
			}
			else
			{
				document.getElementById(field.id+"_td").className="";
				og = false;
			}
			TopdevUtil.rebuildElements();
		}
	...
	ogCheckTab : function(ns, ajaxId, init)
	{
		...
		try
		{
			...
			//alert(init +" - "+TopdevUtil.objs[ns].tabs[ajaxId].og+" - "+newStatus+" - "+oldStatus+" mod:"+TopdevUtil.objs[ns].portletSaveButtonModified);
	...
	{
		document.getElementById(divId).innerHTML = "<br/><table><tr><td><img src='/tis-online-portlet/images/design/loading.gif'/></td><td>"+text+"</td></tr></table><br/>";
	},
	
	toggleSaveButton : function (ns, active)
	{
		//alert(TopdevUtil.objs[ns].portletModified+" - mache save button:"+active);
		if(!TopdevUtil.objs[ns].portletModified) return;
		TopdevUtil.objs[ns].portletSaveButtonModified = true;

		//alert(document.getElementsByName(ns+"_button_save").length);
		...
		if(sbs && sbs.length>0)
		{
			for(i=0; i<sbs.length; i++)
			{
				...
				if(saveButton)
				{
					...
					if(active)	saveButton.className = "portlet-form-button";
					else		saveButton.className = "portlet-form-button_disabled";
				}
			}
		}
		else
	...
	toggleOkButton : function (ns, ajaxId, active)
	{
		...
		//alert("toggleOkButton # "+ajaxId+" # "+ns+" # \n"+okButton+" # "+(okButton?okButton.disabled:""));
		if(okButton)
		{
			okButton.disabled = !active;
			if(active)	okButton.className="portlet-form-button";
			else		okButton.className="portlet-form-button_disabled";
		}
		TopdevUtil.rebuildElements();
	},
	...
	toggleTabOn : function (ns, tabboxId, tabNr)
	{
		...
		document.getElementById(tabboxId+"_tab"	+tabNr).className="tab_active";
