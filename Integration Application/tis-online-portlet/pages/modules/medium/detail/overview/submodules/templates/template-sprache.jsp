		...
		<s:iterator value="entryList" status="s1">
			...
			<tr id="${ajaxId}_${sublistName}_tr<s:property value="#s1.index+1" />" onmouseover="currentTr=this" <c:if test="${inserted}">class="topdev_row_inserted"</c:if>>
				<%@ include file="/pages/lib/fragment/list/direct-v2/body-before.jsp" %>
					...
					<tv2:dropdown
						notd="true"
						serviceName="com.topdev.service.mediumsprache.Client"
				...
				<td>
					<tdl:var-set name="content"><s:property value="sprachergaenzungDeCode"/></tdl:var-set>
					<tdl:var-set name="value"><s:property value="sprachergaenzungCode"/></tdl:var-set>
					<div class="listrow_deleted_div" style="display:${deleted!=1?'none':'inline'}">
						<span><s:property value="sprachergaenzungDeCode"/></span>
					</div>
					<tv2:dropdown
						notd="true"
						serviceName="com.topdev.service.mediumsprachergaenzung.Client"
						name="${sublistName}_sprachergaenzungCode"
						sublistName="${sublistName}"
						labelname="${sublistName}_sprachergaenzungDeCode"
						content="${content}"
						value="${value}"
						style="display:${deleted!=1?'inline;height:20px;':'none'}"/>
				</td>
